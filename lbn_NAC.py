...
class LBN(object):
    ...
    *aux_weights* is used to create the combined feautres. When given, it should have the shape
    ...
    def __init__(self, n_particles, n_restframes=None, n_auxiliaries=None, boost_mode=PAIRS,
            ...
            seed=None, trainable=True, name=None, alpha_weight=1):
...
#         W = getattr(self, name, None)
        W_hat = getattr(self, name, None)
        M_hat = getattr(self, name, None)
        ...
        # define variables
        initializer_w = tf.compat.v1.truncated_normal_initializer(stddev=0.02,seed=seed)
        W_hat = tf.Variable(initializer_w(shape), name=name, trainable=self.trainable)
        initializer_m = tf.compat.v1.truncated_normal_initializer(stddev=0.02,seed=seed)
        M_hat = tf.Variable(initializer_m(shape), name=name, trainable=self.trainable)
...
#         setattr(self, name, W)
        setattr(self, name, W_hat)
        setattr(self, name, M_hat)
...
#         W = getattr(self, name("{}_weights"))
        W_hat = getattr(self, name("{}_weights"))
        M_hat = getattr(self, name("{}_weights"))
...
#         W = tf.identity(W, name=name("final_{}_weights")) #KY
        W = tf.tanh(W_hat, name=name("final_{}_weights"))**2 * tf.sigmoid(M_hat, name=name("final_{}_weights"))
#         W = tf.sigmoid(self.alpha_weight*M_hat, name=name("final_{}_weights"))
        self.W = W
...
class LBNLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, input_shape, *args, **kwargs):
        ...
        self.W = self.lbn.W
        self.W_hat = self.lbn.W_hat
        self.M_hat = self.lbn.M_hat
    ...
    def get_config(self):
        ...
        config.update({
            ...
            "W": self.lbn.W,
            "W_hat": self.lbn.W_hat,
            "M_hat": self.lbn.M_hat,
        })
        return config
